function htmlHome() {
  ...
  return `<div class="hero"><div class="k">SUCHENG / QUICK</div><h2>只打頭碼同尾碼<br>就打到繁體中文。</h2><p class="tiny">速成 = 倉頡簡化版。記住 24 個字根，再練拆頭尾。例如「港」倉頡 <b>水廿金山</b>，速成就係 <b>水山 EU</b>。</p><div class="stats"><div class="stat"><b>${p.streak || 0}</b><span>連續日</span></div><div class="stat"><b>${p.correct || 0}</b><span>拆啲</span></div><div class="stat"><b>${acc(p)}%</b><span>準確率</span></div></div><div class="bar"><i style="width:${Math.min(100, ((p.todayCount || 0) / DAILY_GOAL) * 100)}%"></i></div><p class="tiny">今日目標 ${p.todayCount || 0} / ${DAILY_GOAL}</p></div><div class="path"><button class="task" data-go="next"><div><b>繼續：${next.title}</b><span>${next.blurb}</span></div><em>去</em></button><button class="task" data-go="learn"><div><b>課程路徑</b><span>字根 → 單碼 → 常用 → 粵語 → 限時 → 文章</span></div><em>去</em></button><button class="task" data-go="drill"><div><b>自由拆碼</b><span>常用 / 全部 / 難字 / 錯字本</span></div><em>去</em></button><button class="task" data-go="article"><div><b>文章練習</b><span>內建香港情景，可貼自訂文</span></div><em>去</em></button><button class="task" data-go="review"><div><b>錯字本 ${wrongN}</b><span>打錯自動收，打啲除名</span></div><em>去</em></button><button class="task" data-go="lookup"><div><b>查碼</b><span>漢字 ↔ 速成／倉頡</span></div><em>去</em></button></div>`;
}
function renderHome() {
...
function renderLearn() {
  const p = load();
  $("view-learn").innerHTML = `<h3>跟住條路徑學</h3><p class="tiny">先記字根，再拆碼，最後打整句。</p><div class="path">${LESSONS.map((l) => { const done = p.lessonsDone && p.lessonsDone[l.id]; return `<button class="lesson ${done ? "done" : ""}" data-id="${l.id}"><div><b>${l.title}</b><span>${l.blurb}</span></div><em>${done ? "重溫" : "開始"}</em></button>`; }).join("")}</div>`;
...
function renderRootQuiz() {
  ...
  $("view-learn").querySelectorAll(".choice").forEach((b) => {
    b.onclick = () => {
      ...
      addToday(1);
      toast(ok ? "啲！" : "係 " + q.k + "＝" + SUCHENG.map[q.k]);
...
function renderDrill() {
  ...
  $("view-drill").innerHTML = `<div class="chips">${modes.map(([k, n]) => `<button class="chip ${state.mode === k ? "on" : ""}" data-mode="${k}">${n}</button>`).join("")}<button class="chip ${state.timed.running ? "on" : ""}" id="timed-btn">60秒</button></div>${state.timed.running ? `<div class="timer" id="timer-lab">${state.timed.left}s　${state.timed.ok} 啲</div>` : ""}<div class="drill-card"><div class="tiny">${(modes.find((m) => m[0] === state.mode) || [])[1] || ""} ${state.i + 1}/${list.length}</div><div class="big" id="drill-char">${han}</div><form id="drill-form"><input class="box" id="drill-input" maxlength="2" autocapitalize="off" autocomplete="off" placeholder="頭尾碼，例如 EU" /><div class="row"><button class="btn primary" type="submit">核對</button><button class="btn" type="button" id="drill-show">睇答案</button><button class="btn ghost" type="button" id="drill-next">下一字</button><button class="btn ghost" type="button" id="drill-mark">${marked ? "★" : "☆"}</button></div></form><div class="feedback" id="drill-ans"></div></div><div class="kb" id="drill-kb">${kbHtml("", nextKeys)}</div><p class="tiny rule">只取倉頡<strong>第一碼 + 最後一碼</strong>。錯一次亮頭碼，兩次亮全碼。</p>`;
...
function startTimer(secs) {
  ...
  state.timed.id = setInterval(() => {
    state.timed.left -= 1;
    const lab = $("timer-lab");
    if (lab) lab.textContent = state.timed.left + "s　" + state.timed.ok + " 啲";
    if (state.timed.left <= 0) { stopTimer(); finishLesson("timed"); toast("時間到：啲 " + state.timed.ok); go("home"); }
  }, 1000);
}
function stopTimer() {
...
function renderType() {
  $("view-type").innerHTML = `<div class="chips"><button class="chip on">網頁鍵盤</button><button class="chip" id="to-art">文章練習</button></div><div class="out" id="out"></div><div class="tiny" id="code-now">未入碼</div><div class="cands" id="cands"></div><div class="kb" id="kb">${kbHtml(state.typed, [])}</div><div class="row"><button class="btn" id="type-bs">刪</button><button class="btn" id="type-space">清碼</button><button class="btn" id="copy-out">複製</button><button class="btn ghost" id="type-clear">清空</button></div><p class="tiny">數字鍵 1–9 擇字。空白鍵清碼。</p>`;
...
function renderArticle() {
  ...
  $("view-article").innerHTML = `<div class="chips">${Object.keys(ARTICLES).map((k) => `<button class="chip ${state.article.key === k ? "on" : ""}" data-k="${k}">${ARTICLES[k].title}</button>`).join("")}</div><div class="passage">${chars.map((c, i) => `<span class="${i < state.article.i ? "done" : i === state.article.i ? "cur" : "todo"}">${c}</span>`).join("")}</div><p class="tiny">${art.title}　${state.article.i + 1}/${chars.length}${rec ? "" : "　標點跳過"}</p><input class="box" id="art-input" maxlength="2" autocapitalize="off" autocomplete="off" placeholder="${rec ? "輸入頭尾碼" : "撚下一字"}" /><div class="kb" id="art-kb">${kbHtml("", nextKeys)}</div><div class="row"><button class="btn primary" id="art-ok">${rec ? "核對" : "下一字"}</button><button class="btn" id="art-hint">提示</button><button class="btn ghost" id="art-skip">跳過</button></div><textarea class="box" id="custom-art" placeholder="貼一段繁體中文"></textarea><div class="row"><button class="btn" id="use-custom">用呢段</button></div>`;
...
function renderMe() {
  const p = save();
  $("view-me").innerHTML = `<h3>進度</h3><div class="stats"><div class="stat"><b>${p.streak || 0}</b><span>連續日</span></div><div class="stat"><b>${p.correct || 0}</b><span>拆啲</span></div><div class="stat"><b>${acc(p)}%</b><span>準確率</span></div></div><p class="tiny">打字 ${p.typedChars || 0} 字　今日 ${p.todayCount || 0}/${DAILY_GOAL}　字庫 ${unique(SUCHENG.chars.map((x) => x[0]).filter((h) => h && h.length === 1)).length} 字</p><div class="path"><button class="task" id="go-look"><div><b>查碼</b><span>漢字 ↔ 速成／倉頡</span></div><em>去</em></button><button class="task" id="go-rev"><div><b>錯字本／書籤</b><span>${Object.keys(p.wrongBook || {}).length} 隻錯字</span></div><em>去</em></button></div><button class="btn ghost" id="reset">清除進度</button>`;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  $("goal-pill").onclick = () => go("learn");
  document.addEventListener("keydown", (e) => {
    if (state.view === "drill" && e.key === "Escape") { const b = $("drill-show"); if (b) b.click(); }
