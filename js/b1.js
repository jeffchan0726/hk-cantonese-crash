  ...
  function htmlArticle(){
    ...
    return "<div class='split'><section class='panel'><div class='chips'>"+chips+"</div>"+speed+"<div class='passage'>"+html+"</div>"+imeSlots()+"<form id='art-form'><input id='art-in' type='text' maxlength='2' autocomplete='off' value='"+typed+"'></form><textarea id='art-paste' rows='3' placeholder='貼一篇自己嘅文來打'>"+(state.customArt||"")+"</textarea><div class='row'><button class='btn' id='btn-use-paste' type='button'>用呢篇打</button><button class='ghost' id='btn-skip-art' type='button'>跳過</button></div></section>"+analysisBox(cur||lastHan)+kbHtml(typed)+"</div>";
  }
  function htmlReview(){
  ...
  function htmlMe(){ return "<section class='panel'><h1>進度</h1><div class='stats'><div class='stat'><b>"+(state.streak||0)+"</b><span>連續</span></div><div class='stat'><b>"+(state.correct||0)+"</b><span>拆啲</span></div><div class='stat'><b>"+acc()+"%</b><span>%</span></div></div><button class='ghost' id='btn-reset' type='button'>清進度</button></section>"; }
