  function auxOf(k){
    ...
    return String((SUCHENG.aux&&SUCHENG.aux[k])||SUCHENG.map[k]||k);
  }
  function auxExtra(k){
    k=String(k||"").toUpperCase();
    var main=SUCHENG.map[k]||"";
    return auxOf(k).replace(/\s+/g,"").split("").filter(function(ch){ return ch && ch!==main; }).join("");
  }
  function kbHtml(hintKeys){
    ...
    return "<div class='kb-dock'><div class='kb'>"+KEYS.map(function(row){ return "<div class='kb-row'>"+row.split("").map(function(k){ var cls="k"+(k===first?" hint h0":"")+(last&&k===last&&k!==first?" hint h1":"")+(last&&k===last&&k===first?" h1":""); var extra=(k===first||(last&&k===last))?auxExtra(k):""; return "<button type='button' class='"+cls+"' data-k='"+k.toLowerCase()+"'><b>"+k+"</b><small>"+(SUCHENG.map[k]||"")+"</small>"+(extra?"<span class='aux'>"+extra+"</span>":"")+"</button>"; }).join("")+"</div>"; }).join("")+"</div></div>";
  }
  function wchip(k,i){
    k=String(k||"").toUpperCase();
    var extra=auxExtra(k);
    return "<span class='wchip c"+(i%5)+"'><b>"+(SUCHENG.map[k]||k)+"</b>"+(extra?"<span class='waux'>"+extra+"</span>":"")+"<small>"+k+"</small></span>";
  }
  ...
  function htmlRoots(){ return "<section class='panel'><h1>廿四字根</h1><div class='root-grid'>"+"ABCDEFGHIJKLMNOPQRSTUVWY".split("").map(function(k){ return "<div class='root'><b>"+SUCHENG.map[k]+"</b><span>"+k+"</span><span class='aux'>"+auxOf(k)+"</span></div>"; }).join("")+"</div><div class='row' style='margin-top:16px'><button class='btn' data-go='rootquiz'>測驗</button></div></section>"; }
  ...
  function htmlQuiz(){ if(!quiz) startQuiz(); return "<section class='panel'><h1>「"+SUCHENG.map[quiz.ans]+"」係邊粒鍵？</h1><p class='lede'>"+auxOf(quiz.ans)+"</p><div class='quiz-opts'>"+quiz.opts.map(function(k){ return "<button type='button' data-quiz='"+k+"'>"+k+"<br><small>"+SUCHENG.map[k]+"</small></button>"; }).join("")+"</div></section>"; }
  function htmlDrill(){
    ...
    var bar=drill.timed?("<div class='speedbar'><b id='speed-remain'>"+(remain||0)+"s</b><span id='speed-got'>"+(drill.got||0)+" 字</span><span id='speed-wpm'>"+wpmOf(drill.got,drill.t0)+" 字/分</span></div>"):"";
    var layer=drill.hint===0?"未提示":(drill.hint===1?"第 1 層：頭碼 + 輔助字形":(drill.hint===2?"第 2 層：頭+尾 + 輔助字形":"第 3 層：全拆 + 輔助字形"));
    return "<div class='split'><section class='drill-card wuse'><div class='chips'>"+chips+"</div>"+bar+"<div class='wuse-stage'><div class='char'>"+drill.han+"</div>"+codeChips(drill.han, drill.hint)+"<div class='wlayer'>"+layer+"</div></div><form id='drill-form'><input id='drill-in' type='text' maxlength='2' value='"+String(typed||"").toUpperCase()+"' placeholder='頭碼再尾碼' autocomplete='off'></form><div class='row'><button class='btn' id='btn-check' type='button'>核對</button><button class='ghost' id='btn-hint' type='button'>提示</button><button class='ghost' id='btn-skip' type='button'>跳過</button></div></section>"+kbHtml(hintKeys)+"</div>";
  }
  function htmlArticle(){
    ...
    var layer=hint===0?"對住黃色嗰隻打頭尾碼":(hint===1?"第 1 層：頭碼 + 輔助字形":(hint===2?"第 2 層：頭+尾 + 輔助字形":"第 3 層：全拆 + 輔助字形"));
  ...
  function htmlLookup(){
    ...
    return "<section class='panel'><h1>查碼</h1><input id='look-in' type='text' dir='ltr' value='"+q+"'><div id='look-out'>"+body+"</div><p class='lede'>說 = YU 卜山 · 行 = 彳/丁</p></section>";
  }
  function htmlMe(){ return "<section class='panel'><h1>進度</h1><div class='stats'><div class='stat'><b>"+(state.streak||0)+"</b><span>連續</span></div><div class='stat'><b>"+(state.correct||0)+"</b><span>拆啱</span></div><div class='stat'><b>"+acc()+"%</b><span>%</span></div></div><button class='ghost' id='btn-reset' type='button'>清進度</button></section>"; }
